...
const locations = [
  { name: 'Icarai, Niterói', latitude: -22.9071, longitude: -43.1194 },
...
const findLocationByAddress = (address: string) => {
  return locations.find(location => location.name.toLowerCase() === address.toLowerCase());
};
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  if (req.method === 'POST') {
    ...
    if (!address) {
      return res.status(400).json({ error: 'Endereço não fornecido no corpo da solicitação.' });
    }
